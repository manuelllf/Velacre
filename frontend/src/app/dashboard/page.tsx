      ...
      {hasGenerated && (
        <div className="mx-6 mb-4 bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 rounded-xl p-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Publicar en Google — solo si tiene googleReviewId */}
            {review.googleReviewId && (
              userPlan === 'basic' ? (
                <a
                  href="/settings"
                  className="flex items-center gap-1.5 text-sm font-medium px-4 py-2 border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400 rounded-xl hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors"
                  title="Publicar en Google requiere plan Core o Pro"
                >
                  ...
                  Publicar en Google <span className="text-[10px] font-bold uppercase bg-amber-100 dark:bg-amber-900/40 px-1.5 py-0.5 rounded">Core+</span>
                </a>
              ) : gbpConnected ? (
                review.publicadaEnGoogle ? (
                  <span className="flex items-center gap-1.5 text-sm text-emerald-600 dark:text-emerald-400 font-medium px-4 py-2">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                    Publicada en Google
                  </span>
                ) : (
                  <button
                    onClick={() => onPublish(generated!)}
                ...
                <a
                  href="/settings"
                  className="flex items-center gap-1.5 text-sm font-medium px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                  title="Conecta tu cuenta de Google Business para publicar"
                >
                  <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 opacity-60">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            ...
            {/* Fallback si no tiene googleReviewId (reseña importada por Outscraper sin ID de GBP) */}
            {!review.googleReviewId && (
              <a
