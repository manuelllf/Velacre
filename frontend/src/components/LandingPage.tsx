...
export default function LandingPage({ locale: l }: Props) {
  ...
  const langLinks: Array<{ code: string; label: string; href: string }> = [
    { code: 'es', label: 'ES', href: '/es' },
    { code: 'gal', label: 'GAL', href: '/gal' },
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 pt-24 pb-20 text-center">
          ...
          <p className="text-sm text-slate-600 mt-4">{l.hero.setup}</p>
        </div>
      </section>
                ...
                <div className="space-y-7">
                  ...
                  <div>
                    ...
                    <div className="inline-flex items-center bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                      <button
                        type="button"
                        onClick={() => setCalcPrecioHora(p => Math.max(5, p - 5))}
                        className="px-4 py-2.5 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors text-lg font-bold select-none"
                      >−</button>
                      <span className="px-4 py-2.5 text-white font-black tabular-nums min-w-[80px] text-center text-sm">
                        {calcPrecioHora}€/h
                      </span>
                      <button
                        type="button"
                        onClick={() => setCalcPrecioHora(p => Math.min(500, p + 5))}
                        className="px-4 py-2.5 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors text-lg font-bold select-none"
                      >+</button>
                    </div>
                  </div>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-slate-800 rounded-xl p-4">
                <div className="text-xs text-slate-500 mb-1">{l.health.kpi1}</div>
                ...
                <div className="text-xs text-slate-500">{l.health.prevMonth}</div>
              </div>
              <div className="bg-slate-800 rounded-xl p-4">
                <div className="text-xs text-slate-500 mb-1">{l.health.kpi2}</div>
                <div className="text-3xl font-bold text-white">68%</div>
                <div className="text-xs text-slate-500">{l.health.reviewsOf}</div>
              </div>
              <div className="bg-slate-800 rounded-xl p-4">
                <div className="text-xs text-slate-500 mb-1">{l.health.kpi3}</div>
                <div className="text-3xl font-bold text-white">12</div>
                <div className="text-xs text-slate-500">{l.health.newReviews}</div>
              </div>
            </div>
